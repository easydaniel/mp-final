...
HID_KEYBD_Info_TypeDef     keybd_prev_info;
HID_KEYBD_Info_TypeDef     keybd_info;
HID_KEYBD_LEDS_TypeDef     keybd_leds;
...
USBH_StatusTypeDef USBH_HID_KeybdInit(USBH_HandleTypeDef *phost)
{
  ...
  keybd_leds.CapsLock = 0;
  keybd_leds.NumLock = 1;
  keybd_leds.ScrollLock = 0;
...
const uint8_t numKeys[10] = {'!', '@', '#', '$', '%', '^', '&', '*', '(', ')'};
const uint8_t symKeysUp[12] = {'_', '+', '{', '}', '|', '~', ':', '"', '~', '<', '>', '?'};
const uint8_t symKeysLo[12] = {'-', '=', '[', ']', '\\', ' ', ';', '\'', '`', ',', '.', '/'};
